...
* In order to save the different results in a folder, select a folder path in `path.result` of the function `DEanalysisGlobal()` and set `Save.plots=TRUE`.
...
* `GSEAQuickAnalysis()` requires an internet connection, so the input `Internect.Connection` is set by default to `FALSE`. Once the user is sure to have an internet connection, the user may set `Internect.Connection=TRUE` in order to realize the enrichment analysis.
