...
4. Normalization with `DATAnormalization()`
...
5. Principal component analysis (PCA) with `PCAanalysis()`
...
* **Input**:
  - We recommend the use of the normalized data as input of `ExprData`.
...
6. Hierarchical Clustering on Principle Components (HCPC) with `HCPCanalysis()`
...
7. Temporal clustering analysis with `MFUZZanalysis()`.
...
8. Plot temporal expression with with `DATAplotExpressionGenes()`
...
9. Differential Expresion (DE) analysis with `DEanalysisGlobal()`
...
res.de<-DEanalysisGlobal(RawCounts=Sub3bc3T,
                         Column.gene=1,
                         Group.position=1,
                         Time.position=2,
                         Individual.position=3,
...
10. Volcano and ratio intensity (MA) plots with `DEplotVolcanoMA()`
...
11. Heatmaps with `DEplotHeatmaps()`
...
12. GO enrichment analysis with `GSEAQuickAnalysis()` and `GSEApreprocessing()`

```{r GSEAquickAnalysis_Mouse2, eval=TRUE}
res.gprofiler2<-GSEAQuickAnalysis(Internect.Connection=FALSE,
                                  Res.DE.analysis=res.de,
                                  ColumnsCriteria=2,
