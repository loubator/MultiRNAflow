...
#'
#' @description The function realizes, from the outputs of [DEanalysisGlobal()],
...
#'
#' @param Res.DE.analysis A list corresponding to the output of [DEanalysisGlobal()].
...
#' @param Organism A character indicating the organism where data were taken from.
...
GSEAQuickAnalysis<-function(Res.DE.analysis,
                            ColumnsCriteria=1,
                            ColumnsLog2ordered=NULL,
                            Set.Operation="union",
                            Organism="hsapiens",
  ...
  if(isFALSE(Save.plots)==FALSE){
    ...
    if(is.null(path.result)==FALSE){
      if(Save.plots==TRUE){
        nom.dossier.result1<-paste("2-5-1_gprofiler2_results",SufixDE,sep="")
      }else{
        nom.dossier.result1<-paste("gprofiler2_results",SufixDE,sep="")
      }
      if(nom.dossier.result1%in%dir(path=path.result.f)==FALSE){
        dir.create(path=paste(path.result.f,"/",nom.dossier.result1,sep=""))
        path.result.GSEA<-paste(path.result.f,"/",nom.dossier.result1,sep="")
      }else{
        path.result.GSEA<-paste(path.result.f,"/",nom.dossier.result1,sep="")
      }# if(nom.dossier.result1%in%dir(path = path.result.f)==FALSE)
      LollipopPlot<-paste(path.result.GSEA,"/LollipopChart.pdf",sep="")
      ManhPlot<-paste(path.result.GSEA,"/ManhattanPlot.pdf",sep="")
      GSEAtable<-paste(path.result.GSEA,"/GSEAtable.csv",sep="")
      GSEAtablepng<-paste(path.result.GSEA,"/GSEAtable.pdf",sep="")
    }else{
      # To avoid "no visible binding for global variable" with devtools::check()
  ...
  GeneID<-paste("(",NewgoRes$intersection,")",sep="")
  ...
  GOmat$source<-factor(GOmat$source, levels=c("GO:BP","GO:CC","GO:MF","KEGG"))
  #
  ...
  glolipop<-ggplot2::ggplot(DatLolipop,
                            ggplot2::aes(x=term_id, y=-log10(p_value),
                                         ymin=0, ymax=IntegerMaxLogPval)) +
    ggplot2::geom_point(ggplot2::aes(colour=significant), size=1.9, shape=19)+
    ggplot2::coord_flip() +
    ...
    ggplot2::geom_hline(yintercept=-log10(0.05), linetype="dashed", size=0.6)+
  ...
  #
  DatManhattan<-DatManhattan[order(DatManhattan$source, DatManhattan$p_value),]
  DatManhattan$xmh<-seq_len(nrow(DatManhattan))
  DatManhattan$Color<-factor(DatManhattan$source,
                             levels=c("GO:BP","GO:CC","GO:MF","KEGG"))
  levels(DatManhattan$Color)<-c("#CB2027","#059748","#EE7733","#7B3A96")
  ...
  gManhattan<-ggplot2::ggplot(DatManhattan,
    ...
    ggplot2::scale_color_manual(values=c("#CB2027","#059748",
                                         "#EE7733","#7B3A96"),
                                name="Source")+
    ggplot2::scale_shape_manual(values=c(8,16)[LevelSignificantManh],
                                name="Significant\n   (<0.05)")+
