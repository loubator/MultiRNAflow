...
DEanalysisTimeAndGroup<-function(DESeq.result,
  ...
  if(is.null(path.result)==FALSE){
    ...
    if(Folder_TIMEperBC%in%dir(path = path.result)==FALSE){
      dir.create(path=paste(path.result,"/",Folder_TIMEperBC,sep=""))
      PathResult_TIMEperBC<-paste(path.result,"/",Folder_TIMEperBC,sep="")
    }else{
      PathResult_TIMEperBC<-paste(path.result,"/",Folder_TIMEperBC,sep="")
    }
  }# if(is.null(path.result)==FALSE)
  #
  if(is.null(path.result)==FALSE){
    Folder_BCperTime<-paste("2-2-2_Group_DEanalysis_perTime", sep="")#Other.t
    if(Folder_BCperTime%in%dir(path = path.result)==FALSE){
      dir.create(path=paste(path.result,"/",Folder_BCperTime,sep=""))
      PathResult_BCperTime<-paste(path.result,"/",Folder_BCperTime,sep="")
    }else{
      PathResult_BCperTime<-paste(path.result,"/",Folder_BCperTime,sep="")
    }
  }# if(is.null(path.result)==FALSE)
  ...
  if(is.null(path.result)==FALSE){
    ...
    if(Folder_TIMEandBC%in%dir(path = path.result)==FALSE){
      dir.create(path=paste(path.result,"/",Folder_TIMEandBC,sep=""))
      PathResult_TIMEandBC<-paste(path.result,"/",Folder_TIMEandBC,sep="")
    }else{
      PathResult_TIMEandBC<-paste(path.result,"/",Folder_TIMEandBC,sep="")
    }
  }# if(is.null(path.result)==FALSE)
  ...
  if(is.null(res.dds)==TRUE){
    Row.names.f<-paste("Gene", seq_len(length(res.dds)), sep="")
  }else{
    Row.names.f<-res.dds
  }# if(is.null(res.dds)==TRUE)
  ...
  Id.ref.T<-which(DESeq.result@colData$Time==ref.level.time)
  Vect.T.no.ref<-DESeq.result@colData$Time[-Id.ref.T]
  ...
  colnames(Cont.G.T)<-paste("t_", seq_len(length(Other.t)), sep="")
  #
  M.DE.1tmin<-as.data.frame(matrix(0, nrow=Nb.gene, ncol=Nb.group))
  #
  row.names(M.DE.1tmin)<-Row.names.f
  colnames(M.DE.1tmin)<-rep(paste("DE.1tmin_",Levels.group,sep=""))
  #---------------------------------------------------------------------------#
  ...
  for(g in seq_len(Nb.group)){# 1:Nb.group
    ...
    #
    M.sum.DE.t0.g<-as.data.frame(matrix(data=0,nrow=Nb.gene,
                                        ncol=length(Other.t)*3))
    colnames(M.sum.DE.t0.g)<-paste(rep(c("Log2FoldChange_",
                                         "Pvalue.adjusted_", "DE_"),
                                       times=length(Other.t)),
                                   rep(paste("t",Other.t,".versus.t",
                                             ref.level.time,sep=""),
                                       each=3),"_",
                                   groupe.g, sep="")
    #
    ...
    colnames(M.time.DE.g)<-paste("t", Other.t.num,"_", groupe.g, sep="")
    #
    M.log2FC.g<-matrix(data=0, nrow=Nb.gene, ncol=length(Other.t))
    row.names(M.log2FC.g)<-Row.name.assay
    colnames(M.log2FC.g)<-paste("t", Other.t.num, "_", groupe.g, sep="")
    #
    gene.DE.1tmin.g<-c()
    #-------------------------------------------------------------------------#
    ...
    for(i in seq_len(length(Other.t))){# 1:length(Other.t)
      ...
      Ind.tivst0<-grep(pattern=paste("Time_", i, sep=""),
                       x=DESeq2::resultsNames(DESeq.result))
      if(groupe.g!=ref.level.group){
        Ind.tivst0.g<-grep(pattern=paste("Time",i,".Group",groupe.g,sep=""),
                           x=DESeq2::resultsNames(DESeq.result))
      ...
      #-----------------------------------------------------------------------#
      res.dds.diff.t<-DESeq2::results(DESeq.result, contrast=Index.contrast,
                                      test="Wald")
    ...
    M.sum.DE.t0.g.f<-data.frame(DE.1t.min.g=M.DE.1tmin[,g],
                                Pattern.DE.g=paste(".",Pattern.tps.g,".",
                                                   sep=""),
                                M.sum.DE.t0.g)
    colnames(M.sum.DE.t0.g.f)<-c(paste(c("DE.1time.minimum_","Pattern.DE_"),
                                       groupe.g,sep=""),
    ...
    colnames(M.time.DE.g)<-paste("t", seq_len(ncol(M.time.DE.g)), sep="")
    #
    if(length(Other.t)>1){
      title.alluvial.TG<-paste("Alluvial graph of genes which are DE at",
                               "at least one time,\nfor the group ", groupe.g,
                               sep="")
      title.evolution.TG<-paste("Time evolution of the number of genes which ",
      ...
      names(List.plots.DE.time.group)[2+(g-1)*4+1]<-paste("Alluvial.graph.Group_",
                                                          groupe.g, sep="")
      names(List.plots.DE.time.group)[2+(g-1)*4+2]<-paste("NumberDEgenes.acrossTime",
                                                          ".perTemporalGroup",
                                                          ".Group_",
                                                          groupe.g, sep="")
      names(List.plots.DE.time.group)[2+(g-1)*4+3]<-paste("VennBarplot.Group_",
                                                          groupe.g, sep="")
      names(List.plots.DE.time.group)[2+(g-1)*4+4]<-paste("VennBarplot.",
    ...
    if(is.null(path.result)==FALSE){
      if(length(Other.t)>1){
        # Save alluvial graph
        grDevices::pdf(file=paste(PathResult_TIMEperBC,"/Group_", groupe.g,
                                  "_AlluvialGraph", SubFile.name,".pdf",sep=""),
                       width=11, height=8)
        ...
        # Upset graph
        grDevices::pdf(file=paste(PathResult_TIMEperBC,"/Group_",groupe.g,
                                  "_VennBarplot", SubFile.name,".pdf",sep=""),
                       width=11, height=8)
  ...
  Mat.facet<-data.frame(Attribute=rep(c("UpRegulated", "DownRegulated"),
                        ...
                        Time=rep(rep(paste("t", seq_len(Nb.time-1), sep=""),
                                     each=2),
  ...
  names(List.plots.DE.time.group)[2]<-paste("NumberDEgenes_UpDownRegulated",
                                            "_perTimeperGroup",sep="")
  #---------------------------------------------------------------------------#
  # 2.9) Summary of DE time analysis per group Graph
  #---------------------------------------------------------------------------#
  if(is.null(path.result)==FALSE){
    grDevices::pdf(file=paste(PathResult_TIMEperBC,"/","TimeResultsPerGroup_",
  ...
  Cont.Group.per.Time$Time<-paste("t", gsub("t","", x=Cont.Group.per.Time$Time,
                                            fixed=TRUE), sep="")
  # Color
  if(Nb.group>2){
    Color.Leg.Spe<-data.frame(Levels=c("Other","UpRegulated","DownRegulated"),
  ...
  if(Nb.group>2){
    ...
    names(List.plots.DE.time.group)[Nb.Graph.TperG+1]<-paste("NumberDEgenes_",
                                                             "SpecificAndNoSpecific_",
                                                             "perBiologicalCondition",
                                                             sep="")
    names(List.plots.DE.time.group)[Nb.Graph.TperG+2]<-paste("NumberSpecificGenes_",
                                                             "UpDownRegulated_",
                                                             "perBiologicalCondition",
                                                             sep="")
    names(List.plots.DE.time.group)[Nb.Graph.TperG+3]<-paste("AlluvialGraph_",
                                                             "SpecificGenes1tmin_",
                                                             "perBiologicalCondition",
                                                             sep="")
  }else{
    List.plots.DE.time.group[[Nb.Graph.TperG+1]]<-res.dodge
    names(List.plots.DE.time.group)[Nb.Graph.TperG+1]<-paste("NumberSpecificGenes_",
                                                             "UpDownRegulated_",
                                                             "perBiologicalCondition",
                                                             sep="")
  }# if(Nb.group>2)
  ...
  #---------------------------------------------------------------------------#
  # List.DE.per.pair.G.per.T, List.DE.per.pair.G.per.T, List.DE.per.pair.G.per.T
  if(Nb.group>2){
    for(t in seq_len(Nb.time)){# 1:Nb.time
      G.Upset.t<-DEplotVennBarplotGroup(res.sum.G.T$List.DE.per.pair.G.per.T[[t]])
      #
      Timet<-paste("t",gsub(x=gsub(x=Levels.time[t],pattern="T",replacement=""),
                            pattern="t", replacement = ""), sep="")
      #
      List.plots.DE.time.group[[Nb.Graph.TperG+3+t]]<-G.Upset.t$Upset.global
      names(List.plots.DE.time.group)[Nb.Graph.TperG+3+t]<-paste("VennBarplot_",
      ...
      # Other.t.num<-as.numeric(gsub(x=Other.t, pattern="t", replacement=""))
      if(is.null(path.result)==FALSE){
        grDevices::pdf(file=paste(PathResult_BCperTime,"/",
                                  "VennBarplot_BiologicalConditions_atTime.",
                                  Timet, SubFile.name, ".pdf", sep=""),
                       width=11, height=8)
  ...
  if(is.null(path.result)==FALSE){
    if(Nb.group>2){
      grDevices::pdf(file=paste(PathResult_BCperTime,"/","GroupResultsPerTime_",
  ...
  colnames(Intersect.G.all.t)<-paste("Intersect_", Levels.group, "_T", sep="")
  #
  #---------------------------------------------------------------------------#
  # 4.1) Signature Data
  #---------------------------------------------------------------------------#
  ...
  if(Nb.group>2){
    ...
    #
    title.allu.signature<-paste("Alluvial graph of signature genes at ",
                                "at least one time for each group", sep="")
    res.allu.signature<-DEplotAlluvial(alluvialSignature, FALSE,
  ...
  Mat.facet.S<-data.frame(Attribute=rep(c("UpRegulated", "DownRegulated"),
                          ...
                          Group=rep(rep(Levels.group, each=2),
                                    times=(Nb.time-1)),
                          Time=rep(paste("t", seq_len(Nb.time-1), sep=""),
                                   each=2*Nb.group),
                          value=rep(0,times=2*(Nb.time-1)*Nb.group))
  #
  ...
  Mat.facet.all$Attribute<-as.factor(c(Mat.facet$Attribute,
                                       paste("Signature",Mat.facet.S$Attribute,
                                             sep="")))
  #
  ...
  Color.Leg.S<-data.frame(Levels=c("SignatureUpRegulated", "UpRegulated",
                                   "DownRegulated", "SignatureDownRegulated"),
                          Color=c("#E41A1C", "#EA8626", "#8BEAF0","steelblue"))
  #
  ...
  names(List.plots.DE.time.group)[Nb.GandT+1]<-paste("Number_DEgenes_SignatureGenes_",
                                                     "UpDownRegulated_perTimeperGroup",
                                                     sep="")
  names(List.plots.DE.time.group)[Nb.GandT+2]<-paste("Number_DEgenes1TimeMinimum_",
  ...
  if(Nb.group>2){
    ...
    names(List.plots.DE.time.group)[Nb.GandT+3]<-paste("Alluvial_SignatureGenes_",
  ...
  #---------------------------------------------------------------------------#
  if(is.null(path.result)==FALSE){
    grDevices::pdf(file=paste(PathResult_TIMEandBC, "/",
                              "Number_DEgenes_SignatureGenes_",
    ...
    #
    grDevices::pdf(file=paste(PathResult_TIMEandBC,"/",
                              "Number_DEgenes1TimeMinimum_",
                              "Specific1TimeMinimum_","Signature1TimeMinimum_",
    ...
    if(Nb.group>2){
      grDevices::pdf(file=paste(PathResult_TIMEandBC, "/",
                                "Alluvial_SignatureGenes_",
                                "1TimeMinimum_perGroup", SubFile.name,
                                ".pdf", sep=""),
                     width=11, height=8)
