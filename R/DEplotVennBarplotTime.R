...
DEplotVennBarplotTime<-function(table.DE.time,
    ...
    if(is.null(Log2.FC.matrix)==FALSE){
        ...
        Sign.pat.t<-apply(Sign.DE.t0,
                          MARGIN=1,
                          FUN=function(x) length(which(x==1)))

        Over.Under.pattern<-data.frame(Nb.over=Sign.pat.t,
                                       abs(Sign.DE.t0))
        ...
        SeqCol<-floor(seq(1, length(RainBowCol),
                          length.out=ncol(table.DE.time.DE.gene) + 1))
        ...
        while(length(vectorUniqueValue)>0){
            ...
            mylist.upsetg[[n.over]]<-list(query=UpSetR::elements,
                                          ...
                                          query.name=paste0(as.character(id.over)[n.over],
                                                            ".ti.over.t0"))
