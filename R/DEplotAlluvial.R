...
#' differentially expressed (DE) at time t, compared with the reference time t0.
#' * if \code{Temporal.Group=FALSE} :
#' \eqn{G} columns corresponding to the number of group.
#' A '1' in the \eqn{n}-th row and \eqn{g}-th column means that the n-th gene is
...
#' * is used by the following functions of our package : [DEanalysisTime()] and
...
#'                  ncol=NbTime.vst0)
#' colnames(BinTable)<-paste("t",1:NbTime.vst0,sep="")
#' #
...
DEplotAlluvial<-function(table.DE.time,
  ...
  if(Temporal.Group==TRUE){
    ...
    #-------------------------------------------------------------------------#
    if(is.null(colnames(table.DE.time))==TRUE){
      Dat.col.name<-paste("t", seq_len(ncol(table.DE.time)), sep="")
    }else{
    ...
    if(is.null(row.names(table.DE.time.DE.gene))==TRUE){
    ...
    Id.T.1DE<-which(apply(data.alluvial2.t.ini[,seq_len(ncol(table.DE.time))],
                          2, FUN=function(x) 1%in%x))# 1:ncol(table.DE.time)
    ...
    #
    if(is.null(colnames(table.DE.time))==TRUE){
      label.test.allu<-paste("t", seq_len(ncol(table.DE.time)), sep="")
    }else{
    ...
    q.alluvial<-ggplot2::ggplot(data.alluvial2.tf,
                                ggplot2::aes(x=variable, y=Freq, alluvium=Gene,
                                             fill=Time_group, stratum=value)) +
      ...
      ggalluvial::stat_stratum(width=5, mapping=ggplot2::aes(size=value),
                               fill=Fill.col.bar, reverse=FALSE, alpha=0.7) +
      ggplot2::labs(x="Time", y="Number of genes", fill="Temporal group")+
    ...
    #
    if(is.null(title.alluvial)==TRUE){
    ...
    q.freq.per.time.per.group<-ggplot2::ggplot(data=data.alluvial2,
      ...
      ggplot2::guides(color=ggplot2::guide_legend(title = "Temporal group",
                      ...
                      fill=ggplot2::guide_legend(title = "Temporal group",
                                                 title.position = "left"))+
      ggplot2::geom_line(ggplot2::aes(color=as.character(Category)), size=1)+
      ggplot2::geom_point(ggplot2::aes(color=as.character(Category)), size=2)+
    ...
    #
    if(is.null(title.evolution)==TRUE){
    ...
    # Temporal group is calculated and corresponds to the first time a gene is DE.
    ...
    DE.genes.info<-data.frame(Index=gene.DE.number,
                              ...
                              Attribute=DE.nb.G)
    #-------------------------------------------------------------------------#
    # Data for alluvial graph : Patterns (1 for DE vs t0), time group & Freq
    data.alluvial<-cbind(table.DE.time.DE.gene,
