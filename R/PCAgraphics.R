...
#' will be plotted.
#' These individuals are called supplementary individuals
#' in [FactoMineR::PCA()].
    ...
    if(isTRUE(Plot.PCA) | !is.null(path.result)){
        ...
        if(!is.null(Time.position) & is.null(Group.position)){
            Tlevels<-paste0("t",
                           gsub("t", "",
                                gsub("T", "",
                                     as.character(levels(factor(Vector.time))))))
        ...
        if(!is.null(path.result)){
            ...
            plot3D::scatter3D(data.3D[,1], data.3D[,3], data.3D[,2],
                              ...
                              cex=Cex.point, bty="b2",
                              ticktype = "detailed",
                              theta=Theta, phi=Phi, d=2,
                              clab=c("", data.color$Name),
                              epsilon=epsilon,
                              xlab=paste0("dim1 (",
        ...
        if(!is.null(Vector.time)){
            ...
            if(!is.null(path.result)){
                ...
                if(isFALSE(Mean.Accross.Time) & max(NbPerCond)>1){
                    ...
                    for(col in seq_len(length(levels(ind.color)))){
                        ...
                        Mean.t<-stats::aggregate(res.PCA$ind$coord[Id.col,
                                                                   c(1,2)],
                                                 list(coord.t[Id.col]), mean)
                        New.Name.2D<-apply(unique(data.name[Id.col,]), 1,
                                           paste, collapse="_")

                        graphics::lines(Mean.t[,2],
                                        Mean.t[,3],
                                        type="c", pch=22, lwd=2,
                                        col=levels(ind.color)[col])
                        graphics::text(Mean.t[,2],
                                       Mean.t[,3],
                                       labels=New.Name.2D,
                                       cex=Cex.label*0.9,
                                       col=levels(ind.color)[col])
            ...
            if(!is.null(path.result)){
                ...
                if(isFALSE(Mean.Accross.Time) & max(NbPerCond)>1){
                    ...
                    ##--------------------------------------------------------#
                    for(smpl in seq(from=2,
                                    to=length(levels(factor(Vector.patient))),
                                    by=1)){
