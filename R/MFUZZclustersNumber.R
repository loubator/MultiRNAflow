    ...
    for(g in seq_len(Nb.group)){
        for(t in seq_len(Nb.time)){
            if(is.null(Vect.group)){
                ...
                Index.t<-which(Vect.time==levels(as.factor(Vect.time))[t])
                ExprData.f.TG<-as.data.frame(ExprData.f[,
                                                        intersect(Index.t,Index.g)])
                Data.mfuzz[,Nb.time*(g-1)+t]<-apply(ExprData.f.TG,
                                                    1,
                                                    FUN=mean)
            }# if(is.null(Vect.group)==TRUE)
        }# for(t in 1:Nb.time)
    ...
    if(!is.null(path.result)){
        ...
        if(Nb.group>1){
            for(g in seq_len(Nb.group-1)){
                # Add a second line
                graphics::lines(Sum.nb.c[,1], Sum.nb.c[,g+2]+0.2*g,
                                type="b", lty=g+1,
...
NbClustKmeansHCPC<-function(NrowData,x1,y1,x2,y2){
    if(x2<=x1 | y2<=y1){
        StopMfuzzMessage<-paste0("'x2' must be strictly greater than 'x1' and",
