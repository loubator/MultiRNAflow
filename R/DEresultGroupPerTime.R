...
#' # We take only the first three time (both group) for the speed of the example
...
DEresultGroupPerTime<-function(DESeq.result,
  ...
  for(t in seq_len(Nb.time)){# 1:length(Other.t) # as.numeric(Levels.time)
    ...
    #-------------------------------------------------------------------------#
    # print(paste("DE between biological conditions.", " Time t",t-1,sep=""))
    ...
    for(i in seq_len(Nb.group-1)){# 1:(Nb.group-1)
      for(k in seq(from=(i+1), to=Nb.group, by=1)){# (i+1):Nb.group
        ...
        if(Levels.group[i]==ref.level.group){
          Ind.givsgk<-grep(pattern=paste("_",Levels.group[k],
                                         "_","vs_",Levels.group[i],sep=""),
                           x=DESeq2::resultsNames(DESeq.result))
          #
          if(t==0){
            Index.contrast[Ind.givsgk]<-1
          }else{
            Ind.givsgk.t<-grep(pattern=paste("Time",t-1,".Group",
                                             Levels.group[k],sep=""),
                               x=DESeq2::resultsNames(DESeq.result))#t ->t-1
          ...
          Ind.givsgk.p<-grep(pattern=paste("_",Levels.group[i],
                                           "_","vs_",ref.level.group,sep=""),
                             x=DESeq2::resultsNames(DESeq.result))
          Ind.givsgk.n<-grep(pattern=paste("_",Levels.group[k],
                                           "_","vs_",ref.level.group,sep=""),
                             x=DESeq2::resultsNames(DESeq.result))
          #
          if(t==0){
            ...
            Ind.givsgk.t.p<-grep(pattern=paste("Time",t-1,".Group",
                                               Levels.group[i],sep=""),
                                 x=DESeq2::resultsNames(DESeq.result))#t ->t-1
            Ind.givsgk.t.n<-grep(pattern=paste("Time",t-1,".Group",
                                               Levels.group[k],sep=""),
                                 x=DESeq2::resultsNames(DESeq.result))#t ->t-1
            Index.contrast[c(Ind.givsgk.p,Ind.givsgk.t.p)]<-1
            Index.contrast[c(Ind.givsgk.n,Ind.givsgk.t.n)]<--1
          }# if(t==0)
          fight.group<-paste(".", levels(Vector.group)[i], "..",
                             levels(Vector.group)[k], ".", sep="")
        }# if(Levels.group[i]==ref.level.group)
        #
        res.dds.group<-DESeq2::results(DESeq.result, contrast=Index.contrast,
                                       test="Wald")
        ...
        colnames(Bin.mat.DE)[cpt]<-paste(fight.group,"_t",Levels.time[t],
                                         sep="")#Other.t[t]
        #
    ...
    if(is.null(row.names(res.dds.group))==TRUE){
      Row.name.res<-paste("Gene", seq_len(nrow(res.dds.group)), sep="")
    ...
    #-------------------------------------------------------------------------#
    OverUnder.expr.per.g<-matrix(0, nrow=nrow(res.dds.group), ncol=Nb.group)
    colnames(OverUnder.expr.per.g)<-paste("OverUnder.regulated.genes_Group.",
    ...
    #
    colnames(Resultats.DEseq2.groups)[ColNew]<-paste(colnames(Resultats.DEseq2.groups)[ColNew],
  ...
  colnames(Spe.1t.min)<-paste(colnames(Spe.1t.min), "_1time.minimum", sep="")
  ...
  #
  colnames(M.sum.DE.pair.g.per.t)<-gsub("Log2FoldChange..","Log2FoldChange_",
                                        colnames(M.sum.DE.pair.g.per.t),
                                        fixed=TRUE)
  colnames(M.sum.DE.pair.g.per.t)<-gsub("Pvalue.adjusted..","Pvalue.adjusted_",
                                        colnames(M.sum.DE.pair.g.per.t),
                                        fixed=TRUE)
